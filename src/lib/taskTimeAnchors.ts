/**
 * Strip explicit clock phrases from task text and derive a fixed HH:MM anchor for planning.
 * Used by Clarify and mirrored in generate-plan for raw_input fallback.
 */

const pad = (n: number) => String(Math.max(0, Math.min(23, Math.floor(n)))).padStart(2, "0");

/** Parse hour (0–23) and minute from fragments; respects am/pm when present */
...
/**
 * Removes first matching time expression and optional deadline phrases.
 */
export function extractTaskTimeAnchors(line: string): {
  ...
  // "до 17", "до 17:30", "by 5pm", "until 17:00"
  const deadlineRe =
  ...
  // Standalone meridiem time: "3pm", "11:45 am"
