    ...
    >
      ...
      >
        ...
        <svg width={glyph} height={glyph} viewBox="0 0 120 120" fill="none" className="relative">
          {/* The Lucide fingerprint is authored on a 24×24 grid. Instead of
              stretching it edge-to-edge (scale 5 → fills the whole 120 box and
              looks oversized), centre it at ~64% of the emblem with even
              padding, matching the face glyph's footprint. translate keeps it
              centred: (120 − 24·scale)/2. */}
          <g transform={variant === "fingerprint" ? "translate(21.6 21.6) scale(3.2)" : undefined}>
            {paths.map((d, i) => (
              <motion.path
                key={i}
                d={d}
                stroke={`hsl(${c})`}
                strokeWidth={variant === "fingerprint" ? 1.5 : 5}
                strokeLinecap="round"
                strokeLinejoin="round"
                style={{ filter: `drop-shadow(0 0 ${variant === "fingerprint" ? "1.2px" : "5px"} hsl(${c} / 0.45))` }}
